...
lista_modelos <- list(lm(log_salario ~ age + age^2, data = data_train), #Modelo del punto 3
                   ...
                   lm(log_salario ~ sex + poly(age,3,raw=TRUE):sex  +
                        maxEducLevel + poly(age,3,raw=TRUE):maxEducLevel + oficio*sex + oficio + totalHoursWorked:sex, data = data_train),
                   lm(log_salario ~ sex + poly(age,3,raw=TRUE):sex  +
                   ...
                   lm(log_salario ~ sex + poly(age,4,raw=TRUE):sex  +
                        poly(age,3,raw=TRUE):poly(maxEducLevel,4,raw=TRUE) + poly(maxEducLevel,4,raw=TRUE):sex  +  poly(totalHoursWorked, 3, raw = TRUE):oficio + oficio:poly(maxEducLevel,4,raw=TRUE) + poly(totalHoursWorked, 3, raw = TRUE):sex, data = data_train))
